    ...
    return (
        <header className="sticky top-0 z-50 w-full bg-white border-b border-gray-200">
            ...
            <div className="max-w-[1280px] mx-auto px-5 h-16 grid grid-cols-3 items-center gap-4">
                ...
                {/* Col 1 — Logo (left) */}
                <Link href="/" className="font-extrabold text-2xl tracking-tight text-gray-900 font-[family-name:var(--font-display)]">
                    Megastore
                </Link>
                ...
                <form onSubmit={handleSearch} className="flex items-center border border-gray-200 rounded overflow-hidden w-full">
                    ...
                    />
                    <button type="submit" className="px-4 py-2.5 bg-[#cc1111] flex items-center justify-center shrink-0">
                ...
                <div className="flex items-center gap-1 justify-end">
                    <button
                        onClick={toggleCart}
                        className="p-2 hover:text-[#cc1111] transition-colors relative text-gray-700 flex flex-col items-center gap-0.5 group"
                        title="Cart"
                    >
                        <ShoppingCart className="w-5 h-5" />
                        <span className="text-[9px] font-medium text-gray-400 group-hover:text-[#cc1111] transition-colors hidden sm:block">
                            Cart
                        </span>
                        {itemCount > 0 && (
                            <span className="absolute top-0.5 right-0 w-4 h-4 flex items-center justify-center text-white text-[9px] font-bold rounded-full bg-[#cc1111]">
                                {itemCount}
                            </span>
                        )}
                    </button>
                    ...
                    <nav className="flex items-center">
                        {NAV_LINKS.map((l) => (
                            <Link
                                key={l.label}
                                href={l.href}
                                className="px-5 py-3 text-sm font-semibold transition-colors hover:text-[#cc1111] text-gray-900"
                            >
                                {l.label}
                            </Link>
                        ))}
